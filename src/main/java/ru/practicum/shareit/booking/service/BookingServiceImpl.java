...
public class BookingServiceImpl implements BookingService {
    ...
    public List<BookingDto> getUserBookings(Long userId, State state) {
        ...
        List<Booking> userBookings = switch (state) {
            case CURRENT -> bookingStorage.findCurrentBookingsByUser(userId);
            case WAITING -> bookingStorage.findWaitingBookingsByUser(userId);
            case PAST -> bookingStorage.findPastBookingsByUser(userId);
            case REJECTED -> bookingStorage.findRejectedBookingsByUser(userId);
            case FUTURE -> bookingStorage.findFutureBookingsByUser(userId);
    ...
    public List<BookingDto> getOwnerBookings(Long ownerId, State state) {
        userStorage.findById(ownerId).orElseThrow(() -> new NotFoundException("User not found"));
        List<Booking> userBookings = switch (state) {
            case CURRENT -> bookingStorage.findCurrentBookingsByUser(ownerId);
            case WAITING -> bookingStorage.findWaitingBookingsByUser(ownerId);
            case PAST -> bookingStorage.findPastBookingsByUser(ownerId);
            case REJECTED -> bookingStorage.findRejectedBookingsByUser(ownerId);
            case FUTURE -> bookingStorage.findFutureBookingsByUser(ownerId);
